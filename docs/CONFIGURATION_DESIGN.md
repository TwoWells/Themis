...
TheMan acts as a contractor executing a renovation. The user provides a **Work Order** (Configuration) that specifies:
...
The user should not be burdened with configuring applications they don't use. The `theman.yaml` config simply lists which "Integrations" are active.
