...
## 1. Mechanism
GTK apps (LibAdwaita, GTK3/4) do not read config files directly for theming. They rely on the **DConf** database (GNOME Settings Daemon).
Changes must be made via `gsettings` commands or IPC calls.
**Persistence:** Changes made via `gsettings` are written to disk (`~/.config/dconf/user`) and persist across reboots.
