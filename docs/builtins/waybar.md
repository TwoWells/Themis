...
## 1. Mechanism
Waybar is styled via `style.css`. It supports live reloading if the config changes, OR via `SIGUSR2`.
However, often we only want to change specific colors, not the whole CSS layout.
