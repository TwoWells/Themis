...
The state file is a JSON object updated _after_ a successful `load` operation.
...
4.  Look for a corresponding "inverse" preset?
    - _Option A (Naming Convention):_ Look for `nord-light` if current is `nord-dark`.
    - _Option B (Variable Toggle):_ Load the _same_ preset `nord`, but inject `mode: light` as an
      override.
...
inside the YAML based on a flag. _Solution:_ A Preset can define "Variants" or we simply rely on
