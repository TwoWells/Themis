...
Integrations are the logic units that apply a theme to a specific application. In "TheMan" analogy,
these are the **Workers** (Painter, Electrician, Carpenter).

## 1. Integration Structure

An integration is defined by a **Manifest**. This can be defined inline in `theman.yaml` or in
separate files (e.g., `~/.config/theman/integrations/foot.yaml`).
...
The most powerful method. TheMan renders a template file (e.g., `theme.j2`) into a target
configuration file.
...
- **Use Case:** `alacritty.toml` imports `theme.toml`. TheMan symlinks `theme.toml` to
  `presets/nord.toml`.
